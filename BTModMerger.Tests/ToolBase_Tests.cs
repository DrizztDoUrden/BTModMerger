...
public class ToolBase_Tests
{
    ...
    [Fact]
    public static void SaveResult_Null()
    {
        var tmp = Path.GetTempFileName();
        File.Delete(tmp);
        ToolBase.SaveResult(tmp, null);
        Assert.False(File.Exists(tmp));
    }

    [Fact]
    public static void SaveResult_Empty()
    {
        var tmp = Path.GetTempFileName();
        File.Delete(tmp);
        ToolBase.SaveResult(tmp, new());
        Assert.False(File.Exists(tmp));
    }
    ...
    {
        var tmp = Path.GetTempFileName();
        ToolBase.SaveResult(tmp, new(new XElement("e")));
        Assert.True(File.Exists(tmp));
        Assert.Equal(new XDocument(new XElement("e")), XDocument.Load(tmp), XNode.DeepEquals);
        File.Delete(tmp);
    }

    [Fact]
    public static void SaveResult_MissingDirectory()
    {
        var tmp = Path.GetTempFileName();
        File.Delete(tmp);
        var tmp2 = Path.Combine(tmp, "123.xml");
        ToolBase.SaveResult(tmp2, new(new XElement("e")));
